 ...
 * Роли прототипа.
 *
 * На этой стадии заказчик принимает две плашки — клиент и руководитель
 * (решение Р-140). Эксперт и менеджер остаются в матрице прав и в схеме,
 * но в прототипе не показываются: их сценарии смотрятся отдельно, когда
 * до них дойдёт черёд.
 */
const ROLES = [
