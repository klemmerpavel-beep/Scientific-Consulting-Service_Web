...
function entryPage(snapshotHtml) {
  ...
  return `<div class="pt-entry">
    <p class="pt-entry__lead">Прототип: письма он не шлёт, поэтому ссылка входа заменена выбором роли.
    Состав разделов и содержимое экранов у каждой роли свои — это не оформление, а матрица прав.</p>
...
const ENTRY_NOTE = {
  client: 'свои работы, этапы, материалы, переписка, счета',
  expert: 'назначенные работы и собственное вознаграждение',
  manager: 'очередь заявок, реестры, сроки',
  head: 'всё перечисленное, плюс деньги, аналитика, журналы, справочники',
};
