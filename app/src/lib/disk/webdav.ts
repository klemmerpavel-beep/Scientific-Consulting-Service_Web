 ...
 * положить файл, забрать файл, убрать файл, — и ради них не стоит вводить
...
export class YandexDisk {
  ...
  /**
   * Убрать файл из папки зеркала. Отсутствие файла — не ошибка: цель
   * достигнута.
   *
   * Это не уничтожение. WebDAV DELETE на Яндекс Диске кладёт файл в
   * Корзину, и там он лежит до её очистки (решение Р-252). Удалить насовсем
   * умеет только REST API Диска (`DELETE /v1/disk/resources?permanently=true`),
   * а он требует OAuth-токена приложения; у зеркала есть лишь логин и пароль
   * приложения для WebDAV, и REST с ними не работает. Поэтому после
   * требования субъекта об удалении Корзину диска очищают руками — экран
   * зеркала об этом говорит.
   */
