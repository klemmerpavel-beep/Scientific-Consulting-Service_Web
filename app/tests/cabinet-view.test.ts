...
describe('облик держится в границах дизайн-системы', () => {
  ...
  for (const folder of ['client', 'expert', 'manager', 'head']) {
    for (const file of screens(folder)) {
      ...
      // Таблица шире экрана прокручивается вбок. Мышью и пальцем это
      // работает само, с клавиатуры — только когда у области прокрутки
      // есть фокус и имя. Проверка доступности нашла тринадцать таких
      // экранов, где ни того, ни другого не было (Р-168).
      it(`${name}: область прокрутки достижима с клавиатуры`, () => {
        for (const m of body(file).matchAll(/<(\w+)\b([^>]*overflow-x:\s*auto[^>]*)>/gu)) {
