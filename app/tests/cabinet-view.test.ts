...
describe('снимок не зависит от времени съёмки', () => {
  /**
   ...
   * подряд расходились на случайных экранах, а разбор изменений показывал
   * шум вместо правок облика (решение Р-186).
   */
  for (const folder of ['client', 'expert', 'manager', 'head']) {
    for (const file of screens(folder)) {
      it(path.relative(PROTOTYPE, file), () => {
        ...
        assert.equal(
          /class="pd-progress"/u.test(html),
          false,
          'в снимок попал след движения — снимок перестал быть воспроизводимым',
        );
      });
    }
  }
