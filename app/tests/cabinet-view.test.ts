...
describe('график не остаётся единственным носителем числа', () => {
  ...
  for (const file of charts) {
    ...
    it(`${name}: у каждого графика есть числа текстом`, () => {
      const html = body(file).match(/<main\b[\s\S]*?<\/main>/u)?.[0] ?? '';
      // Карточка графика — секция с заголовком, графиком и числами. Ищем
      // проще и надёжнее: в разметке экрана с графиком обязана быть либо
      // таблица, либо перечень легенды со значениями.
      const numbers =
        (html.match(/<table\b/gu) ?? []).length + (html.match(/<li\b[^>]*>[\s\S]{0,400}?%<\/span>/gu) ?? []).length;
      assert.ok(numbers > 0, 'числа графика существуют только картинкой');
    });
  }
});
