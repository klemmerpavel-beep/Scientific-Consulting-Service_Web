...
describe('экраны кабинета не пишут разметку форм заново', () => {
  for (const file of sources(SCREENS)) {
    ...
    // Область, которая прокручивается вбок, обязана получать фокус, иначе
    // правые колонки таблицы недоступны с клавиатуры (Р-168). Прокрутка
    // заведена в `TableCard`, и объявлять её на экране больше незачем.
    it(`${name}: прокрутка таблицы собрана компонентом`, () => {
      assert.equal(
        /overflowX/u.test(code),
        false,
        'на экране своя область прокрутки вместо TableCard',
      );
    });
