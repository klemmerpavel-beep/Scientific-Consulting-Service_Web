...
const aside = [
  ...
  // Копия прототипа для боевого сайта (решение Р-174). В витрине прототип
  // уже публикуется из `design/` тем же кодом, и вторая его копия добавила
  // бы к артефакту 2,6 МБ ради тех же самых снимков.
  ['public/cabinet-preview', `${shelf}/cabinet-preview`],
];
...
try {
  ...
  // Второй рубеж защиты от индексации. Мета-тег `noindex, nofollow` на
  // страницах ставит layout по NEXT_PUBLIC_DEMO_STAND, но маршрут robots.ts
  // на время сборки убран (он серверный), и `robots.txt` иначе не появится
