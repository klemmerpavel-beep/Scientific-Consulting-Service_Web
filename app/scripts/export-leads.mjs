 ...
 * обращения. Файл хранится на сервере или на машине оператора и удаляется,
 * как только перестал быть нужен. Зарубежные хранилища исключены: это
 * трансграничная передача, которой Политика не допускает. Российское
 * хранилище по договору поручения обработки — отдельный порядок с
 * собственным зеркалом (Р-164), и эта ручная выгрузка к нему отношения
 * не имеет: её файл кладётся туда, куда решит оператор.
 *
...
try {
  ...
  if (arg === '--stdout') {
    ...
    console.error('В файле персональные данные: хранить на сервере, в чужое облако не выкладывать.');
  }
} finally {
